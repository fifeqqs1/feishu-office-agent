...
const toolSelectorText = computed(() => (
  selectedTools.value.length > 0
    ? `手动限定 ${selectedTools.value.length} 个`
    : (plugins.value.length > 0 ? `自动选择 ${plugins.value.length} 个工具` : '自动选择工具')
))

const mcpSelectorText = computed(() => (
  selectedMcpServers.value.length > 0
    ? `手动限定 ${selectedMcpServers.value.length} 个MCP`
    : (mcpServers.value.length > 0 ? `自动选择 ${mcpServers.value.length} 个MCP` : '自动选择MCP')
))
...
const generateSessionId = (): string => {
  // 局域网 HTTP 访问时，部分浏览器环境可能没有 crypto.randomUUID
    ...
    <div class="chat-container">
      ...
      <div v-if="messages.length === 0" class="welcome-section">
        <div class="avatar-wrapper">
          <img src="/ai.svg" alt="饺子 Agent" class="avatar" />
        </div>
        <h1 class="welcome-title">我是饺子 Agent 小助手，很高兴见到你！</h1>
        <p class="welcome-subtitle">
          欢迎体验饺子 Agent，开启您的智能协作新纪元~
        </p>
      </div>
            ...
            <div class="footer-left">
              ...
              <div class="selector-dropdown" ref="toolDropdownRef">
                ...
                >
                  ...
                  <span class="selector-text">{{ toolSelectorText }}</span>
                  <span class="selector-arrow">▲</span>
                </div>
                ...
                <transition name="dropdown">
                  <div v-if="showToolSelector" class="dropdown-menu tool-menu">
                    ...
                    <div class="dropdown-header">
                      ...
                      <span class="header-count">{{ plugins.length }} 个可用，默认按意图自动挑选</span>
                    </div>
              ...
              <div class="selector-dropdown" ref="mcpDropdownRef">
                ...
                >
                  ...
                  <span class="selector-text">{{ mcpSelectorText }}</span>
                  <span class="selector-arrow">▲</span>
                </div>
                ...
                <transition name="dropdown">
                  <div v-if="showMcpSelector" class="dropdown-menu tool-menu">
                    ...
                    <div class="dropdown-header">
                      ...
                      <span class="header-count">{{ mcpServers.length }} 个可用，默认按意图自动挑选</span>
                    </div>
